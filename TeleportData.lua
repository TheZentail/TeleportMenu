TeleportData = TeleportData or {}

TeleportData["Current Season"] = {
    ["Dungeons"] = {
        [1] = "Algeth'ar Academy",
        [2] = "Azure Vault",
        [3] = "Brackenhide Hollow",
        [4] = "Dawn of the Infinite",
        [5] = "Halls of Infusion",
        [6] = "Neltharus",
        [7] = "Nokhud Offensive",
        [8] = "Ruby Life Pools",
        [9] = "Uldaman: Legacy of Tyr"
    },
    ["Raids"] = {
        [1] = "Aberrus, the Shadowed Crucible",
        [2] = "Amirdrassil, the Dream's Hope",
        [3] = "Vault of the Incarnates"
    }
}

TeleportData["Pathways"] = {
    [1] = {
        ["Expansion"] = "Classic"
    },
    [2] = {
        ["Expansion"] = "Burning Crusade"
    },
    [3] = {
        ["Expansion"] = "Wrath of the Lich King"
    },
    [4] = {
        ["Expansion"] = "Cataclysm",
        ["Dungeons"] = {
            [1] = {
                ["Name"] = "Grim Batol",
                ["Spell"] = "Path of the Grim Batol"
            },
            [2] = {
                ["Name"] = "Throne of the Tides",
                ["Spell"] = "Path of the Tidehunter"
            },
            [3] = {
                ["Name"] = "Vortex Pinnacle",
                ["Spell"] = "Path of Wind's Domain"
            }
        }
    },
    [5] = {
        ["Expansion"] = "Mists of Pandaria",
        ["Dungeons"] = {
            [1] = {
                ["Name"] = "Gate of the Setting Sun",
                ["Spell"] = "Path of the Setting Sun"
            },
            [2] = {
                ["Name"] = "Mogu'shan Palace",
                ["Spell"] = "Path of the Mogu King"
            },
            [3] = {
                ["Name"] = "Scarlet Halls",
                ["Spell"] = "Path of the Scarlet Blade"
            },
            [4] = {
                ["Name"] = "Scarlet Monastery",
                ["Spell"] = "Path of the Scarlet Mitre"
            },
            [5] = {
                ["Name"] = "Scholomance",
                ["Spell"] = "Path of the Necromancer"
            },
            [6] = {
                ["Name"] = "Shadow-Pan Monastery",
                ["Spell"] = "Path of the Shado-Pan"
            },
            [7] = {
                ["Name"] = "Siege of Niuzao Temple",
                ["Spell"] = "Path of the Black Ox"
            },
            [8] = {
                ["Name"] = "Stormstout Brewery",
                ["Spell"] = "Path of the Stout Brew"
            },
            [9] = {
                ["Name"] = "Temple of the Jade Serpent",
                ["Spell"] = "Path of the Jade Serpent"
            }
        }
    },
    [6] = {
        ["Expansion"] = "Warlords of Draenor",
        ["Dungeons"] = {
            [1] = {
                ["Name"] = "Auchindoun",
                ["Spell"] = "Path of the Vigilant"
            },
            [2] = {
                ["Name"] = "Bloodmaul Slag Mines",
                ["Spell"] = "Path of the Bloodmaul"
            },
            [3] = {
                ["Name"] = "Everbloom",
                ["Spell"] = "Path of the Verdant"
            },
            [4] = {
                ["Name"] = "Grimrail Depot",
                ["Spell"] = "Path of the Dark Rail"
            },
            [5] = {
                ["Name"] = "Iron Docks",
                ["Spell"] = "Path of the Iron Paw"
            },
            [6] = {
                ["Name"] = "Shadowmoon Burial Grounds",
                ["Spell"] = "Path of the Crescent Moon"
            },
            [7] = {
                ["Name"] = "Skyreach",
                ["Spell"] = "Path of the Skies"
            },
            [8] = {
                ["Name"] = "Upper Blackrock Spire",
                ["Spell"] = "Path of the Burning Mountain"
            }
        }
    },
    [7] = {
        ["Expansion"] = "Legion",
        ["Dungeons"] = {
            [1] = {
                ["Name"] = "Black Rook Hold",
                ["Spell"] = "Path of Ancient Horrors"
            },
            [2] = {
                ["Name"] = "Court of Stars",
                ["Spell"] = "Path of the Grand Magistrix"
            },
            [3] = {
                ["Name"] = "Darkheart Thicket",
                ["Spell"] = "Path of the Nightmare Lord"
            },
            [4] = {
                ["Name"] = "Halls of Valor",
                ["Spell"] = "Path of Proven Worth"
            },
            [5] = {
                ["Name"] = "Karazhan",
                ["Spell"] = "Path of the Fallen Guardian"
            },
            [6] = {
                ["Name"] = "Neltharion's Lair",
                ["Spell"] = "Path of the Earth-Warder"
            }
        }
    },
    [8] = {
        ["Expansion"] = "Battle for Azeroth",
        ["Dungeons"] = {
            [1] = {
                ["Name"] = "Atal'Dazar",
                ["Spell"] = "Path of the Golden Tomb"
            },
            [2] = {
                ["Name"] = "Freehold",
                ["Spell"] = "Path of the Freebooter"
            },
            [3] = {
                ["Name"] = "Operation: Mechagon",
                ["Spell"] = "Path of the Scrappy Prince"
            },
            [4] = {
                ["Name"] = "Siege of Boralus",
                ["Spell"] = "Path of the Besieged Harbor"
            },
            [5] = {
                ["Name"] = "Underrot",
                ["Spell"] = "Path of Festering Rot"
            },
            [6] = {
                ["Name"] = "Waycrest Manor",
                ["Spell"] = "Path of Heart's Bane"
            }
        }
    },
    [9] = {
        ["Expansion"] = "Shadowlands",
        ["Dungeons"] = {
            [1] = {
                ["Name"] = "De Other Side",
                ["Spell"] = "Path of the Scheming Loa"
            },
            [2] = {
                ["Name"] = "Halls of Atonement",
                ["Spell"] = "Path of the Sinful Soul"
            },
            [3] = {
                ["Name"] = "Mists of Tirna Scithe",
                ["Spell"] = "Path of the Misty Forest"
            },
            [4] = {
                ["Name"] = "Necrotic Wake",
                ["Spell"] = "Path of the Courageous"
            },
            [5] = {
                ["Name"] = "Plaguefall",
                ["Spell"] = "Path of the Plagued"
            },
            [6] = {
                ["Name"] = "Sanguine Depths",
                ["Spell"] = "Path of the Stone Warden"
            },
            [7] = {
                ["Name"] = "Spires of Ascension",
                ["Spell"] = "Path of the Ascendant"
            },
            [8] = {
                ["Name"] = "Tazavesh, the Veiled Market",
                ["Spell"] = "Path of the Streetwise Merchant"
            },
            [9] = {
                ["Name"] = "Theater of Pain",
                ["Spell"] = "Path of the Undefeated"
            }
        },
        ["Raids"] = {
            [1] = {
                ["Name"] = "Castle Nathria",
                ["Spell"] = "Path of the Sire"
            },
            [2] = {
                ["Name"] = "Sanctum of Domination",
                ["Spell"] = "Path of the Tormented Soul"
            },
            [3] = {
                ["Name"] = "Seplucher of the First Ones",
                ["Spell"] = "Path of the First Ones"
            }
        }
    },
    [10] = {
        ["Expansion"] = "Dragonflight",
        ["Dungeons"] = {
            [1] = {
                ["Name"] = "Algeth'ar Academy",
                ["Spell"] = "Path of the Draconic Diploma"
            },
            [2] = {
                ["Name"] = "Azure Vault",
                ["Spell"] = "Path of Arcane Secrets"
            },
            [3] = {
                ["Name"] = "Brackenhide Hollow",
                ["Spell"] = "Path of the Rotting Woods"
            },
            [4] = {
                ["Name"] = "Dawn of the Infinite",
                ["Spell"] = "Path of Twisted Time"
            },
            [5] = {
                ["Name"] = "Halls of Infusion",
                ["Spell"] = "Path of the Titanic Reservoir"
            },
            [6] = {
                ["Name"] = "Neltharus",
                ["Spell"] = "Path of the Obsidian Hoard"
            },
            [7] = {
                ["Name"] = "Nokhud Offensive",
                ["Spell"] = "Path of the Windswept Plains"
            },
            [8] = {
                ["Name"] = "Ruby Life Pools",
                ["Spell"] = "Path of the Clutch Defender"
            },
            [9] = {
                ["Name"] = "Uldaman: Legacy of Tyr",
                ["Spell"] = "Path of the Watcher's Legacy"
            }
        },
        ["Raids"] = {
            [1] = {
                ["Name"] = "Aberrus, the Shadowed Crucible",
                ["Spell"] = "Path of the Bitter Legacy"
            },
            [2] = {
                ["Name"] = "Amirdrassil, the Dream's Hope",
                ["Spell"] = "Path of the Scorching Dream"
            },
            [3] = {
                ["Name"] = "Vault of the Incarnates",
                ["Spell"] = "Path of the Primal Prison"
            }
        }
    },
    [11] = {
        ["Expansion"] = "The War Within",
        ["Dungeons"] = {
            [1] = {
                ["Name"] = "Path of the Ara-Kara, City of Echoes",
                ["Spell"] = "Path of the Ara-Kara, City of Echoes"
            },
            [2] = {
                ["Name"] = "Path of the Brewery",
                ["Spell"] = "Path of the Brewery"
            },
            [3] = {
                ["Name"] = "Path of the City of Threads",
                ["Spell"] = "Path of the City of Threads"
            },
            [4] = {
                ["Name"] = "Path of the Darkflame Cleft",
                ["Spell"] = "Path of the Darkflame Cleft"
            },
            [5] = {
                ["Name"] = "Path of the Dawnbreaker",
                ["Spell"] = "Path of the Dawnbreaker"
            },
            [6] = {
                ["Name"] = "Path of the Priory of the Sacred Flame",
                ["Spell"] = "Path of the Priory of the Sacred Flame"
            },
            [7] = {
                ["Name"] = "Path of the Rookery",
                ["Spell"] = "Path of the Rookery"
            },
            [8] = {
                ["Name"] = "Path of the Stonevault",
                ["Spell"] = "Path of the Stonevault"
            }
        }
    }
}

TeleportData["Hearthstones"] = {
    ["Brewfest Reveler's Hearthstone"] = 166747,
    ["Broker Translocation Matrix"] = 190237,
    ["Dark Portal"] = 93672,
    ["Deepdweller's Earthen Hearthstone"] = 208704,
    ["Dominated Hearthstone"] = 162973,
    ["Enlightened Hearthstone"] = 190196,
    ["Eternal Traveler's Hearthstone"] = 172179,
    ["Ethereal Portal"] = 54452,
    ["Fire Eater's Hearthstone"] = 166746,
    ["Greatfather Winter's Hearthstone"] = 162973,
    ["Headless Horseman's Hearthstone"] = 163045,
    ["Hearthstone"] = 6948,
    ["Hearthstone of the Flame"] = 209035,
    ["Holographic Digitalization Hearthstone"] = 168907,
    ["The Innkeeper's Daughter"] = 64488,
    ["Kyrian Hearthstone"] = 184353,
    ["Lunar Elder's Hearthstone"] = 165669,
    ["Necrolord Hearthstone"] = 182773,
    ["Night Fae Hearthstone"] = 180290,
    ["Noble Gardener's Hearthstone"] = 165802,
    ["Ohn'ir Windsage's Hearthstone"] = 200630,
    ["Peedlefeet's Lovely Hearthstone"] = 165670,
    -- ["Stone of the Hearth"] = 212337, -- currently not sharing cooldown with others
    ["Timewalker's Hearthstone"] = 193588,
    ["Venthyr Sinstone"] = 183716
}

TeleportData["ClassPorts"] = {
    ["Mage"] = {
        ["Teleports"] = {
            [1] = {
                ["Spell"] = "Teleport: Darnassus",
                ["Location"] = "Darnassus",
                ["SpellID"] = 3561
            },
            [2] = {
                ["Spell"] = "Teleport: Exodar",
                ["Location"] = "Exodar",
                ["SpellID"] = 32266
            },
            [3] = {
                ["Spell"] = "Teleport: Ironforge",
                ["Location"] = "Ironforge",
                ["SpellID"] = 3562
            },
            [4] = {
                ["Spell"] = "Teleport: Orgrimmar",
                ["Location"] = "Orgrimmar",
                ["SpellID"] = 3567
            },
            [5] = {
                ["Spell"] = "Teleport: Silvermoon",
                ["Location"] = "Silvermoon",
                ["SpellID"] = 32272
            },
            [6] = {
                ["Spell"] = "Teleport: Stormwind",
                ["Location"] = "Stormwind",
                ["SpellID"] = 3561
            },
            [7] = {
                ["Spell"] = "Teleport: Theramore",
                ["Location"] = "Theramore",
                ["SpellID"] = 49359
            },
            [8] = {
                ["Spell"] = "Teleport: Thunder Bluff",
                ["Location"] = "Thunder Bluff",
                ["SpellID"] = 3566
            },
            [9] = {
                ["Spell"] = "Teleport: Undercity",
                ["Location"] = "Undercity",
                ["SpellID"] = 3563
            },
            [10] = {
                ["Spell"] = "Teleport: Stonard",
                ["Location"] = "Stonard",
                ["SpellID"] = 49361
            },
            [11] = {
                ["Spell"] = "Teleport: Shattrath",
                ["Location"] = "Shattrath",
                ["SpellID"] = 33691
            },
            [12] = {
                ["Spell"] = "Teleport: Dalaran - Northrend",
                ["Location"] = "Dalaran - Northrend",
                ["SpellID"] = 53140
            },
            [13] = {
                ["Spell"] = "Ancient Teleport: Dalaran",
                ["Location"] = "Ancient Dalaran",
                ["SpellID"] = 120146
            },
            [14] = {
                ["Spell"] = "Teleport: Tol Barad",
                ["Location"] = "Tol Barad",
                ["SpellID"] = 88342
            },
            [15] = {
                ["Spell"] = "Teleport: Vale of Eternal Blossoms",
                ["Location"] = "Vale of Eternal Blossoms",
                ["SpellID"] = 132621
            },
            [16] = {
                ["Spell"] = "Teleport: Stormshield",
                ["Location"] = "Stormshield",
                ["SpellID"] = 176246
            },
            [17] = {
                ["Spell"] = "Teleport: Warspear",
                ["Location"] = "Warspear",
                ["SpellID"] = 176248
            },
            [18] = {
                ["Spell"] = "Teleport: Dalaran - Broken Isles",
                ["Location"] = "Dalaran - Broken Isles",
                ["SpellID"] = 224869
            },
            [19] = {
                ["Spell"] = "Teleport: Boralus",
                ["Location"] = "Boralus",
                ["SpellID"] = 281403
            },
            [20] = {
                ["Spell"] = "Teleport: Dazar'alor",
                ["Location"] = "Dazar'alor",
                ["SpellID"] = 281404
            },
            [21] = {
                ["Spell"] = "Teleport: Oribos",
                ["Location"] = "Oribos",
                ["SpellID"] = 344587
            },
            [22] = {
                ["Spell"] = "Teleport: Valdrakken",
                ["Location"] = "Valdrakken",
                ["SpellID"] = 395277
            }
        },
        ["Portals"] = {
            [1] = {
                ["Spell"] = "Portal: Darnassus",
                ["Location"] = "Darnassus",
                ["SpellID"] = 11419
            },
            [2] = {
                ["Spell"] = "Portal: Exodar",
                ["Location"] = "Exodar",
                ["SpellID"] = 32267
            },
            [3] = {
                ["Spell"] = "Portal: Ironforge",
                ["Location"] = "Ironforge",
                ["SpellID"] = 11416
            },
            [4] = {
                ["Spell"] = "Portal: Orgrimmar",
                ["Location"] = "Orgrimmar",
                ["SpellID"] = 11417
            },
            [5] = {
                ["Spell"] = "Portal: Silvermoon",
                ["Location"] = "Silvermoon",
                ["SpellID"] = 32271
            },
            [6] = {
                ["Spell"] = "Portal: Stormwind",
                ["Location"] = "Stormwind",
                ["SpellID"] = 10059
            },
            [7] = {
                ["Spell"] = "Portal: Theramore",
                ["Location"] = "Theramore",
                ["SpellID"] = 49360
            },
            [8] = {
                ["Spell"] = "Portal: Thunder Bluff",
                ["Location"] = "Thunder Bluff",
                ["SpellID"] = 11420
            },
            [9] = {
                ["Spell"] = "Portal: Undercity",
                ["Location"] = "Undercity",
                ["SpellID"] = 11418
            },
            [10] = {
                ["Spell"] = "Portal: Stonard",
                ["Location"] = "Stonard",
                ["SpellID"] = 49362
            },
            [11] = {
                ["Spell"] = "Portal: Shattrath",
                ["Location"] = "Shattrath",
                ["SpellID"] = 33690
            },
            [12] = {
                ["Spell"] = "Portal: Dalaran - Northrend",
                ["Location"] = "Dalaran - Northrend",
                ["SpellID"] = 53142
            },
            [13] = {
                ["Spell"] = "Ancient Portal: Dalaran",
                ["Location"] = "Ancient Dalaran",
                ["SpellID"] = 120145
            },
            [14] = {
                ["Spell"] = "Portal: Tol Barad",
                ["Location"] = "Tol Barad",
                ["SpellID"] = 88345
            },
            [15] = {
                ["Spell"] = "Portal: Vale of Eternal Blossoms",
                ["Location"] = "Vale of Eternal Blossoms",
                ["SpellID"] = 132620
            },
            [16] = {
                ["Spell"] = "Portal: Stormshield",
                ["Location"] = "Stormshield",
                ["SpellID"] = 176248
            },
            [17] = {
                ["Spell"] = "Portal: Warspear",
                ["Location"] = "Warspear",
                ["SpellID"] = 176249
            },
            [18] = {
                ["Spell"] = "Portal: Dalaran - Broken Isles",
                ["Location"] = "Dalaran - Broken Isles",
                ["SpellID"] = 224871
            },
            [19] = {
                ["Spell"] = "Portal: Boralus",
                ["Location"] = "Boralus",
                ["SpellID"] = 281404
            },
            [20] = {
                ["Spell"] = "Portal: Dazar'alor",
                ["Location"] = "Dazar'alor",
                ["SpellID"] = 281404
            },
            [21] = {
                ["Spell"] = "Portal: Oribos",
                ["Location"] = "Oribos",
                ["SpellID"] = 344589
            },
            [22] = {
                ["Spell"] = "Portal: Valdrakken",
                ["Location"] = "Valdrakken",
                ["SpellID"] = 395289
            }
        }
    }
}

TeleportData["DruidPorts"] = {
    --
}

TeleportData["MonkPorts"] = {
    -- zen pilgrimage
}

TeleportData["Engineering"] = {
    ["Shadowlands Engineering"] = {
        {
            ["ItemID"] = 180732,
            ["ItemName"] = "Wormhole Generator: Shadowlands",
            ["Display"] = "Shadowlands Wormhole",
            ["SkillReq"] = 1
        }
    }
}

TeleportData["Other"] = {
    ["Items"] = {
        {
            ["Name"] = "Admiral's Compass",
            ["ItemID"] = 128353,
            ["Location"] = "Garrison Shipyard"
        },
        {
            ["Name"] = "Direbrew's Remote",
            ["ItemID"] = 37863,
            ["Location"] = "Blackrock Depths"
        },
        {
            ["Name"] = "Lucky Tortollan Charm",
            ["ItemID"] = 202046,
            ["Location"] = "Stormsong Valley"
        },
        {
            ["Name"] = "The Last Relic of Argus",
            ["ItemID"] = 64457,
            ["Location"] = "Random EK/Kalimdor"
        }
    },
    ["Toys"] = {
        {
            ["Name"] = "Dalaran Hearthstone",
            ["ItemID"] = 140192,
            ["Location"] = "Dalaran - Broken Isles"
        },
        {
            ["Name"] = "Garrison Hearthstone",
            ["ItemID"] = 110560,
            ["Location"] = "Garrison"
        }
    },
    ["Equips"] = {
        {
            ["Name"] = "Band of the Kirin Tor",
            ["ItemID"] = 40586,
            ["Location"] = "Dalaran - Northrend"
        },
        {
            ["Name"] = "Cloak of Coordination",
            ["ItemID"] = 65274,
            ["Location"] = "Orgrimmar"
        },
        {
            ["Name"] = "Cloak of Coordination",
            ["ItemID"] = 65360,
            ["Location"] = "Stormwind"
        },
        {
            ["Name"] = "Ruby Slippers",
            ["ItemID"] = 28585,
            ["Location"] = "Hearthstone"
        }
    }
}
